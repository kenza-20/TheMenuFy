...
const addReservation = async (req, res) => {
  const { userId, dishId, date, time, specialRequests, numberOfGuests, userEmail } = req.body;

  try {
    if (!userId || !dishId || !date || !time || !numberOfGuests || !userEmail) {
    ...
    await sendConfirmationEmail(userEmail, { date, time, specialRequests, numberOfGuests });
    ...
    console.error(error);
    res.status(500).json({ error: 'Something went wrong while creating the reservation' });
  }
};
...
const getReservationsByUser = async (req, res) => {
  ...
  try {
    const reservations = await Reservation.find({ userId }).populate('dishId');
...
module.exports = { signupUser,login_post,logout,forgotPassword,resetPassword,updateMonProfil,addReservation,getReservationsByUser,getByToken };
