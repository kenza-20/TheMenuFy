...
const login_post = async (req, res) => {
    const { email, password, tableId } = req.body; // Accept tableId in the login request
    ...
    try {
        ...
        // Verify if the user's account is approved
        ...
        // Check if a valid tableId is passed in the request
        if (tableId) {
            ...
            if (table.isOccupied) {
                return res.status(400).json({ message: 'Cette table est déjà occupée.' });
            }

            // Assign the tableId to the user and mark the table as occupied
            user.tableId = table._id; // Assign tableId to the user
            table.isOccupied = true; // Mark the table as occupied
            await table.save(); // Save the updated table
            await user.save(); // Save the updated user with tableId
        }

        // Generate JWT token
        const token = jwt.sign(
            { id: user._id, email: user.email, role: user.role, tableId: user.tableId || null },
        ...
        res.json({
            ...
            user: {
                ...
                email: user.email,
                role: user.role,
                tableId: user.tableId || null // Return tableId in the response
            }
        });
