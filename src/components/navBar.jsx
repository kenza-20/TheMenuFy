...
const Navbar = ({ authenticated }) => {
  ...
  // Masquer navbar sur pages login/register/reset
  if (
  ...
  const isMenuPage = location.pathname.match(/^\/resto\/\d+\/menu$/);
      ...
      <div className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 space-x-8">
        {!authenticated ? (
          ...
          <>
            ...
            <Link to="/scan" className="text-white hover:text-yellow-500">Scanner QR</Link>
          </>
        )}
      </div>

      {/* Desktop : Recherche + Profil */}
      <div className="hidden md:flex items-center space-x-4">

        {/* Recherche visible uniquement sur /resto/:id/menu */}
        {isMenuPage && (
          <input
            type="text"
            ...
            className="px-4 py-1 rounded-full text-sm bg-white/10 text-white border border-yellow-500 placeholder:text-yellow-200 outline-none"
          />
        )}
