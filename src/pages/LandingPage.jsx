...
function LandingPage() {
  ...
  const [menuItemss, setMenuItems] = useState([]); // Top seller
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        // Fetch the top-selling dish
        const response = await axios.get(
          "http://localhost:3000/api/dish/top-sellers"
        );
        console.log("Top Seller Response:", response);

        if (response.data) {
          const data = Array.isArray(response.data) ? response.data : [response.data];
          setMenuItems(data);

          // Set the main image if there are any menu items
          if (data.length > 0) {
            ...
            const similarResponse = await axios.get(
              `http://localhost:3000/api/dish/${category}/similar`
            );
            setSimilarDishes(similarResponse.data);
          }
        } else {
        ...
        <div className="w-full max-w-7xl pt-15">
          ...
          >
            <div className="flex flex-col space-y-10">
              ...
              <h3 className="text-2xl font-bold text-white">TOP -SELLER</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-4">
              ...
              <div className="mt-10">
                <h2 className="text-2xl font-bold text-white">Similar Dishes</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-4">
