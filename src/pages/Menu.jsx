...
const Menu = () => {
  ...
  const [challengeItems, setChallengeItems] = useState(null);
  ...
  const fetchChallengeDish = async () => {
    try {
      const res = await axios.get('http://localhost:3000/api/dish/challenge');
  ...
  const ChallengeCard = () => (
    <div className="bg-green-100/10 border border-green-400/20 p-6 rounded-xl text-white shadow-lg">
      <h3 className="text-xl font-bold text-green-300 mb-2">🎯 Challenge du jour</h3>
      <p className="text-sm text-gray-200 mb-2">
        Pas de soda pendant 24h. Offrez-vous une salade vitaminée ! 🥗
      </p>
      <p className="text-green-400 font-semibold text-sm">
        Petit geste, grande énergie ✨
      </p>
    </div>
  );
      ...
      <main className="relative flex-grow flex items-center justify-center py-6 px-4 sm:px-6 lg:px-20">
        <div className="w-full max-w-7xl mx-auto">
            ...
            {categories.map(category => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-medium ${
                  activeCategory === category && !challengeItems
             ...
             <button
              onClick={fetchChallengeDish}
              className={`px-6 py-2 rounded-full text-sm font-medium ${
                challengeItems ? 'bg-green-500 text-white' : 'bg-white/10 text-green-400 hover:bg-green-400/20'
              }`}
            >
              Daily Challenge
            </button>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8">
            ...
            {challengeItems && <ChallengeCard />}
            {(challengeItems || menuItems.filter(item => item.category === activeCategory)).map(item => (
