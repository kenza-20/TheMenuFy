...
const Menu = () => {
  ...
  const addToCart = (item) => {
    ...
    const isConfirmed = window.confirm(`Do you want to add ${item.name} to your cart?`);
  
    if (!isConfirmed) {
      setPopupMessage(`You canceled adding: ${item.name}`);
      setShowPopup(true);
      setTimeout(() => {
        setShowPopup(false);
      }, 3000);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="absolute inset-0 bg-cover bg-center bg-no-repeat -z-10" style={{ backgroundImage: "url('/login1.jpg')", boxShadow: "inset 0 0 0 2000px rgba(0, 0, 0, 0.3)" }} />
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredMenuItems.map(item => (
                <div key={item.id} className="bg-white/5 rounded-xl p-6 border border-white/10 hover:bg-white/10 transition-colors">
                  <img
                    src={item.image}
                    alt={item.name}
