...
const OrderHistory = () => {
      ...
      try {
        ...
        const formattedOrders = response.data.map((order: any) => {
          ...
          return {
            ...
            // Correctly handle group order information
            isGroupOrder: order.isGroupOrder === true, // Ensure boolean conversion
            groupOrderCode: order.groupOrderCode,
            groupOrderName:
              order.groupOrderName || (order.groupOrderCode ? `Group Order ${order.groupOrderCode}` : null),
            groupOrderPaymentMethod: order.groupOrderPaymentMethod,
            userPaymentAmount: order.userPaymentAmount || order.total,
            groupOrderTotalAmount: order.groupOrderTotalAmount,
          }
        })
  ...
  const generateInvoice = (order: Order) => {
    ...
    if (order.isGroupOrder) {
      doc.text(`Group Order: ${order.groupOrderName}`, 14, startY)
      doc.text(`Group Order Code: ${order.groupOrderCode}`, 14, startY + 5)

      if (order.groupOrderPaymentMethod === "split") {
        doc.text(`Payment Method: Split Payment (Your Share)`, 14, startY + 10)
        doc.text(`Group Total: $${order.groupOrderTotalAmount?.toFixed(2)}`, 14, startY + 15)
        startY += 20
      } else if (order.groupOrderPaymentMethod === "individual") {
        doc.text(`Payment Method: Individual Payment (Your Items)`, 14, startY + 10)
        doc.text(`Group Total: $${order.groupOrderTotalAmount?.toFixed(2)}`, 14, startY + 15)
        startY += 20
      } else {
        doc.text(`Payment Method: Single Payment (Full Amount)`, 14, startY + 10)
        startY += 15
      }
    }
  ...
  // Function to view group order details
  const viewGroupOrder = (code: string) => {
    if (code) {
                      ...
                      >
                        <div className="flex flex-wrap justify-between items-center">
                          <div className="flex items-center space-x-4">
                            <div
                              className={`p-2 rounded-full ${order.isGroupOrder ? "bg-blue-500/20" : "bg-yellow-500/20"}`}
                            >
                              {order.isGroupOrder ? (
                                <Users
                                  className={`h-6 w-6 ${order.isGroupOrder ? "text-blue-500" : "text-yellow-500"}`}
                                />
                              ) : (
                              ...
                              <h3 className="text-lg font-semibold text-white">
                                {order.isGroupOrder ? (
                                  <span className="flex items-center">
                                    Group Order: {order.groupOrderName}
                          ...
                          <div className="flex items-center space-x-4">
                            <div className="text-right">
                              <div className="flex items-center text-yellow-500 font-semibold">
                                ...
                                {order.isGroupOrder &&
                                  order.groupOrderTotalAmount &&
                                  order.groupOrderTotalAmount > order.total && (
                                    <span className="text-white/70 text-xs ml-1">
                                      of ${order.groupOrderTotalAmount.toFixed(2)}
                                    </span>
                                  )}
                              </div>
                                ...
                                <div className="mb-4 p-3 bg-blue-500/10 rounded-lg border border-blue-500/20">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                    <div className="text-white/80">
                                      <span className="text-white/60">Name:</span> {order.groupOrderName}
                                    </div>
                                    <div className="text-white/80">
                                      <span className="text-white/60">Code:</span> {order.groupOrderCode}
                                    </div>
                                    <div className="text-white/80">
                                      <span className="text-white/60">Payment Method:</span>{" "}
                                      {order.groupOrderPaymentMethod === "split"
                                        ? "Split Payment"
                                        : order.groupOrderPaymentMethod === "individual"
                                          ? "Individual Payment"
                                          : "Single Payment"}
                                    </div>
                                    {order.groupOrderTotalAmount && (
                                      <div className="text-white/80">
                                        <span className="text-white/60">Group Total:</span> $
                                        {order.groupOrderTotalAmount.toFixed(2)}
                                      </div>
                                    )}
                                    {order.userPaymentAmount && (
                                      <div className="text-white/80">
                                        <span className="text-white/60">Your Payment:</span> $
                                        {order.userPaymentAmount.toFixed(2)}
                                      </div>
                                    )}
                                  </div>
                                  {order.groupOrderCode && (
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation()
                                        viewGroupOrder(order.groupOrderCode!)
                                      }}
                                      className="mt-2 flex items-center text-blue-500 hover:text-blue-400 text-sm"
                                    >
                                      <span>View Group Order</span>
