...
import GroupOrderButton from "../components/group-order/group-order-button"
...
const voiceOrder = location.state?.voiceOrder
console.log("📦 voiceOrder reçu :", voiceOrder)

  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/api/orders/${id_user}`)

        const transformedMeals = res.data.map((item) => ({
          id: item.price_id,
          name: item.name,
        ...
        console.log(transformedMeals, "ordersss")
...
useEffect(() => {
  if (voiceOrder?.items?.length > 0) {
    console.log("📦 Déclenché avec voiceOrder :", voiceOrder)

    const updatedMeals = [...meals]
    const updatedQuantities = { ...quantities }

    voiceOrder.items.forEach((item) => {
      // ✅ Utiliser un fallback ID si `price_id` est manquant
      const fallbackId = item.price_id || `voice-${item.name?.toLowerCase().replace(/\s+/g, '-')}`

      if (!item.name || !item.price || !item.quantity) {
        console.warn("⚠️ Donnée vocale incomplète ignorée :", item)
        return
      }

      const existingIndex = updatedMeals.findIndex((m) => m.id === fallbackId)

      if (existingIndex !== -1) {
        // ✅ Déjà présent → on cumule les quantités
        updatedQuantities[fallbackId] = (updatedQuantities[fallbackId] || 0) + item.quantity
      } else {
        // ✅ Nouveau plat
        updatedMeals.push({
          id: fallbackId,
          name: item.name,
          price: item.price,
          description: `Commande vocale : ${item.name}`,
          image: "/placeholder.svg",
        })
        updatedQuantities[fallbackId] = item.quantity
      }
    })
